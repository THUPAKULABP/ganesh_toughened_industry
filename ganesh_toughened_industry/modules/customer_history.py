...
class CustomerHistoryModule:
    ...
    def search_history(self):
        ...
        try:
            # This is a simplified query - in a real implementation, you would filter by date
            query = """
            SELECT i.*, c.name as customer_name, c.place as customer_place
            FROM invoices i
    ...
    def create_invoice_details_ui(self, parent, invoice):
        ...
        ttk.Label(inv_num_frame, text="Invoice #:").pack(side=tk.LEFT, width=15)
        ...
        ttk.Label(date_frame, text="Date:").pack(side=tk.LEFT, width=15)
        ttk.Label(date_frame, text=invoice["date"]).pack(side=tk.LEFT)
        
        # Customer
        cust_frame = ttk.Frame(details_frame)
        cust_frame.pack(fill=tk.X, padx=5, pady=2)
        ttk.Label(cust_frame, text="Customer:").pack(side=tk.LEFT, width=15)
        ...
        ttk.Label(place_frame, text="Place:").pack(side=tk.LEFT, width=15)
        ...
        ttk.Label(pay_mode_frame, text="Payment Mode:").pack(side=tk.LEFT, width=15)
        ...
        ttk.Label(ppay_frame, text="P-PAY No.:").pack(side=tk.LEFT, width=15)
    ...
    def create_payment_history_ui(self, parent, customer_id):
        ...
        try:
            ...
            total_invoices = self.cursor.fetchone()["total_invoices"]
            ...
            total_payments = self.cursor.fetchone()["total_payments"]
    ...
    def load_outstanding(self):
        ...
        try:
            ...
            for customer in customers:
                # Calculate outstanding
                outstanding = self.db.get_customer_outstanding(customer["customer_id"])
                
                # Only show customers with outstanding balance
                if outstanding > 0:
                    # Get total invoices and payments
                    self.cursor = self.db.conn.cursor()
                    self.cursor.execute("""
                    SELECT COALESCE(SUM(total), 0) as total_invoices
                    FROM invoices WHERE customer_id = ?
    ...
    def view_customer_details(self, event=None):
        ...
        ttk.Label(name_frame, text="Name:").pack(side=tk.LEFT, width=10)
        ttk.Label(name_frame, text=customer["name"]).pack(side=tk.LEFT)
        
        # Place
        place_frame = ttk.Frame(details_frame)
        place_frame.pack(fill=tk.X, padx=5, pady=2)
        ttk.Label(place_frame, text="Place:").pack(side=tk.LEFT, width=10)
        ...
        ttk.Label(phone_frame, text="Phone:").pack(side=tk.LEFT, width=10)
        ...
        ttk.Label(gst_frame, text="GST:").pack(side=tk.LEFT, width=10)
        ...
        ttk.Label(addr_frame, text="Address:").pack(side=tk.LEFT, width=10)
        ...
        history_btn = ttk.Button(action_frame, text="View History", 
                                command=lambda: self.view_customer_history(customer))
