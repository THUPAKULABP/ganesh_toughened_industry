...
class InventoryModule:
    ...
    def create_movements_tab(self, parent):
        ...
        edit_btn = ttk.Button(action_frame, text="Edit Movement", command=self.edit_movement)
        edit_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Delete movement button
        delete_btn = ttk.Button(action_frame, text="Delete Movement", command=self.delete_movement)
        delete_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Bind selection event
        self.movements_tree.bind("<<TreeviewSelect>>", self.on_movement_select)
        
        # Bind keyboard delete
        self.movements_tree.bind("<Delete>", lambda e: self.delete_movement())
    ...
    def on_product_select(self, event):
        ...
        if selection:
            # Enable edit and delete buttons when a product is selected
            pass  # Buttons are always enabled in this implementation
    
    def on_movement_select(self, event):
        """Handle movement selection in the treeview"""
        selection = self.movements_tree.selection()
        if selection:
            # Enable edit and delete buttons when a movement is selected
            pass  # Buttons are always enabled in this implementation
    ...
    def search_movements(self):
        ...
        try:
            ...
            for movement in movements:
                # Add to treeview with inventory_id as item ID
                self.movements_tree.insert("", "end", iid=str(movement["inventory_id"]), values=(
                    movement["date"],
    ...
    def edit_movement(self):
        """Edit selected stock movement"""
        selected = self.movements_tree.selection()
        if not selected:
            messagebox.showinfo("Info", "Please select a movement to edit")
            return
        
        # Get inventory ID from selection (convert to integer)
        try:
            inventory_id = int(selected[0])
        except ValueError:
            messagebox.showerror("Error", "Invalid inventory ID")
            return
        
        # Get movement details from treeview
        item = self.movements_tree.item(selected[0])
        movement_date_str = item["values"][0]
        product_name = item["values"][1]
        movement_type = item["values"][2]
        quantity_str = item["values"][3]
        notes = item["values"][4]
        
        # Parse date from string (format: YYYY-MM-DD)
        try:
            year, month, day = movement_date_str.split('-')
            movement_date = date(int(year), int(month), int(day))
        except ValueError:
            messagebox.showerror("Error", "Invalid date format in movement record")
            return
        ...
        dialog = tk.Toplevel(self.parent)
        dialog.title("Edit Stock Movement")
        dialog.geometry("400x300")
        dialog.transient(self.parent)
        dialog.grab_set()
        ...
        # Movement details
        ttk.Label(dialog, text="Date:").grid(row=0, column=0, padx=10, pady=5, sticky=tk.W)
        date_var = tk.StringVar(value=movement_date.strftime("%d/%m/%Y"))
        ttk.Entry(dialog, textvariable=date_var).grid(row=0, column=1, padx=10, pady=5, sticky=tk.EW)
        
        ttk.Label(dialog, text="Product:").grid(row=1, column=0, padx=10, pady=5, sticky=tk.W)
        product_var = tk.StringVar(value=product_name)
        product_combo = ttk.Combobox(dialog, textvariable=product_var, state="readonly")
        product_combo.grid(row=1, column=1, padx=10, pady=5, sticky=tk.EW)
        ...
        ttk.Label(dialog, text="Type:").grid(row=2, column=0, padx=10, pady=5, sticky=tk.W)
        type_var = tk.StringVar(value=movement_type)
        type_combo = ttk.Combobox(dialog, textvariable=type_var, values=["stock_in", "stock_out"], state="readonly")
        type_combo.grid(row=2, column=1, padx=10, pady=5, sticky=tk.EW)
        
        ttk.Label(dialog, text="Quantity:").grid(row=3, column=0, padx=10, pady=5, sticky=tk.W)
        quantity_var = tk.StringVar(value=quantity_str)
        ttk.Entry(dialog, textvariable=quantity_var).grid(row=3, column=1, padx=10, pady=5, sticky=tk.EW)
        ...
        def save_movement():
            try:
                ...
                date_str = date_var.get()
                day, month, year = date_str.split('/')
                new_movement_date = date(int(year), int(month), int(day))
                ...
                # Get product ID - FIXED: Use the original product_id from the movement record
                # Get the current product_id from the database using the movement record
                current_movement = self.db.get_daily_inventory(new_movement_date)
                current_product_id = None
                
                # Find the current movement in the list
                for movement in current_movement:
                    if movement["inventory_id"] == inventory_id:
                        current_product_id = movement["product_id"]
                        break
                
                # If we can't find the current movement, try to find by product name
                if current_product_id is None:
                    for product in products:
                        if f"{product['name']} ({product['type']})" == selected_product:
                ...
                # Get type
                new_movement_type = type_var.get()
                if not new_movement_type:
                    messagebox.showerror("Error", "Please select a movement type")
                    return
                ...
                # Update movement
                success = self.db.update_inventory(
                    inventory_id=inventory_id,
                    product_id=current_product_id,
                    date=new_movement_date,
                    type=new_movement_type,
                    quantity=quantity,
                    notes=notes_var.get()
                )
                
                if success:
                    messagebox.showinfo("Success", "Stock movement updated successfully")
                    dialog.destroy()
                    # Refresh movements
                    self.search_movements()
                    # Refresh current inventory
                    ...
                    messagebox.showerror("Error", "Could not update stock movement")
                
            except ValueError:
                messagebox.showerror("Error", "Please enter valid values")
            except Exception as e:
                messagebox.showerror("Error", f"Could not update stock movement: {e}")
    ...
    def delete_movement(self):
        """Delete selected stock movement"""
        selected = self.movements_tree.selection()
        if not selected:
            messagebox.showinfo("Info", "Please select a movement to delete")
            return
        ...
        if messagebox.askyesno("Confirm Delete", 
                              f"Are you sure you want to delete this {movement_type} of {quantity} for {product_name}? "
                              f"The movement will be moved to the recycle bin."):
            # Delete movement from database (moves to recycle bin)
            success = self.db.delete_inventory(inventory_id)
            ...
            if success:
                ...
                messagebox.showinfo("Success", "Stock movement moved to recycle bin")
            else:
                messagebox.showerror("Error", "Could not delete stock movement")
    ...
    def load_products(self):
        ...
        try:
            ...
            for product in products:
                # Use product_id as the item ID (convert to string for treeview)
                self.products_tree.insert("", "end", iid=str(product["product_id"]), values=(
                    product["name"],
                    product["type"],
    ...
    def edit_product(self):
        ...
        # Get product ID from selection (convert to integer)
        try:
        ...
        # Get product details from treeview
        ...
        ttk.Label(dialog, text="Type:").grid(row=1, column=0, padx=10, pady=5, sticky=tk.W)
        type_var = tk.StringVar(value=product_type)
        type_combo = ttk.Combobox(dialog, textvariable=type_var, values=["Plain", "Toughened", "Laminated"], state="readonly")
        type_combo.grid(row=1, column=1, padx=10, pady=5, sticky=tk.EW)
        
        ttk.Label(dialog, text="Rate per SQ.FT:").grid(row=2, column=0, padx=10, pady=5, sticky=tk.W)
        rate_var = tk.StringVar(value=rate_str)
        ttk.Entry(dialog, textvariable=rate_var).grid(row=2, column=1, padx=10, pady=5, sticky=tk.EW)
    ...
    def delete_product(self):
        ...
        if messagebox.askyesno("Confirm Delete", 
                              ...
                              f"The product will be moved to the recycle bin."):
            # Delete product from database (moves to recycle bin)
            result, message = self.db.delete_product(product_id)
