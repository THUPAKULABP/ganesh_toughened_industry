...
class AttendanceModule:
    ...
    def create_daily_tab(self, parent):
        ...
        # Attendance treeview
        self.attendance_tree = ttk.Treeview(tree_frame, columns=("worker", "morning", "afternoon", "notes"), show="headings")
        ...
        self.attendance_tree.bind("<Double-1>", self.edit_notes)
        
        # Action buttons
        action_frame = ttk.Frame(main_frame)
        action_frame.pack(fill=tk.X, padx=5, pady=5)
        
        # Mark morning present button
        morning_present_btn = ttk.Button(action_frame, text="Mark Morning Present", command=self.mark_morning_present)
        morning_present_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Mark afternoon present button
        afternoon_present_btn = ttk.Button(action_frame, text="Mark Afternoon Present", command=self.mark_afternoon_present)
        afternoon_present_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Mark morning absent button
        morning_absent_btn = ttk.Button(action_frame, text="Mark Morning Absent", command=self.mark_morning_absent)
        morning_absent_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Mark afternoon absent button
        afternoon_absent_btn = ttk.Button(action_frame, text="Mark Afternoon Absent", command=self.mark_afternoon_absent)
        afternoon_absent_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
    ...
    def edit_notes(self, event=None):
        """Edit notes for selected worker"""
        ...
        values = list(self.attendance_tree.item(selected[0])["values"])
        worker_name = values[0]
        current_notes = values[3]
        
        # Create notes dialog
        notes_dialog = tk.Toplevel(self.parent)
        notes_dialog.title(f"Edit Notes for {worker_name}")
        notes_dialog.geometry("400x200")
        notes_dialog.transient(self.parent)
        notes_dialog.grab_set()
        
        ttk.Label(notes_dialog, text="Enter notes for this worker:").pack(pady=10)
        
        notes_entry = ttk.Entry(notes_dialog, width=40)
        ...
        def save_notes():
            ...
            # Update treeview
            self.attendance_tree.item(selected[0], values=(
                values[0],  # Worker name
                values[1],  # Morning
                values[2],  # Afternoon
        ...
        ttk.Button(button_frame, text="Cancel", command=notes_dialog.destroy).pack(side=tk.LEFT, padx=5)
    ...
    def create_workers_tab(self, parent):
        ...
        # Create a frame for the treeview and scrollbar
        tree_frame = ttk.Frame(workers_frame)
        tree_frame.pack(fill=tk.BOTH, expand=True)
        
        # Workers treeview
        self.workers_tree = ttk.Treeview(tree_frame, columns=("name", "phone"), show="headings")
        ...
        # Add scrollbar
        scrollbar = ttk.Scrollbar(tree_frame, orient=tk.VERTICAL, command=self.workers_tree.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
    ...
    def create_calendar_tab(self, parent):
        ...
        # Load workers for the dropdown BEFORE loading the calendar view
        self.load_workers()
    ...
    def mark_morning_present(self):
        """Mark selected worker as present in the morning"""
        selected = self.attendance_tree.selection()
        if not selected:
    ...
    def mark_afternoon_present(self):
        """Mark selected worker as present in the afternoon"""
        selected = self.attendance_tree.selection()
        if not selected:
            messagebox.showinfo("Info", "Please select a worker")
            return
    ...
    def mark_morning_absent(self):
        """Mark selected worker as absent in the morning"""
        selected = self.attendance_tree.selection()
        if not selected:
            messagebox.showinfo("Info", "Please select a worker")
            return
    ...
    def mark_afternoon_absent(self):
        """Mark selected worker as absent in the afternoon"""
        selected = self.attendance_tree.selection()
        if not selected:
            messagebox.showinfo("Info", "Please select a worker")
            return
    ...
    def save_attendance(self):
        ...
        for item in items:
            ...
            # If notes is empty, prompt for notes
            if not notes:
                # Get worker name
                worker_name = values[0]
                
                # Create a dialog to add notes
                notes_dialog = tk.Toplevel(self.parent)
                notes_dialog.title(f"Add Notes for {worker_name}")
                notes_dialog.geometry("400x200")
                notes_dialog.transient(self.parent)
                notes_dialog.grab_set()
                ...
                ttk.Button(button_frame, text="Save Notes", command=save_notes).pack(side=tk.LEFT, padx=5)
                ttk.Button(button_frame, text="Skip", command=skip_notes).pack(side=tk.LEFT, padx=5)
                
                # Wait for dialog to close
                notes_dialog.wait_window()
            else:
                # Save to database
                self.db.add_attendance(
                    worker_id=worker_id,
    ...
    def delete_worker(self):
        ...
        # Confirm deletion
        if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete {worker['name']}? The worker will be moved to the recycle bin."):
            # Delete worker from database (moves to recycle bin)
            ...
            if success:
                ...
                messagebox.showinfo("Success", "Worker moved to recycle bin")
            else:
    ...
    def load_calendar_view(self):
        ...
        for i, day in enumerate(day_names):
            day_label = ttk.Label(header_frame, text=day, width=10, relief=tk.RIDGE, borderwidth=1)
            day_label.grid(row=0, column=i, padx=2, pady=2)
        
        # Create a separate frame for the calendar grid
        calendar_grid = ttk.Frame(self.calendar_frame)
        calendar_grid.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Create calendar grid using grid geometry manager
        for week_idx, week in enumerate(cal):
            for day_idx, day in enumerate(week):
                if day == 0:  # Day not in month
                    day_label = ttk.Label(calendar_grid, text="", width=10, relief=tk.RIDGE, borderwidth=1)
                    day_label.grid(row=week_idx, column=day_idx, padx=2, pady=2, sticky="nsew")
                else:
                    # Create day frame
                    day_frame = ttk.Frame(calendar_grid, relief=tk.RIDGE, borderwidth=1)
                    day_frame.grid(row=week_idx, column=day_idx, padx=2, pady=2, sticky="nsew")
                    ...
                    if worker_id:
                        ...
                        if attendance:
                            ...
                            if attendance["morning"]:
                                morning_label = ttk.Label(att_frame, text="M: ✓", foreground="green")
                            else:
                                morning_label = ttk.Label(att_frame, text="M: ✗", foreground="red")
                            morning_label.pack(side=tk.LEFT)
                            
                            # Afternoon attendance
                            if attendance["afternoon"]:
                                afternoon_label = ttk.Label(att_frame, text="A: ✓", foreground="green")
                            else:
                                afternoon_label = ttk.Label(att_frame, text="A: ✗", foreground="red")
                            afternoon_label.pack(side=tk.LEFT)
                            
                            # Notes if available
                            if attendance["notes"]:
                                notes_label = ttk.Label(day_frame, text="📝", font=("TkDefaultFont", 8))
                                notes_label.pack()
                                
                                # Add tooltip for notes
                                def show_notes(event, notes=attendance["notes"]):
                                    tooltip = tk.Toplevel()
                                    tooltip.wm_overrideredirect(True)
                                    tooltip.wm_geometry(f"+{event.x_root+10}+{event.y_root+10}")
                                    label = ttk.Label(tooltip, text=notes, background="lightyellow", relief=tk.SOLID, borderwidth=1)
                                    label.pack()
                                ...
                                notes_label.bind("<Enter>", show_notes)
        
        # Configure grid weights to make cells expand properly
        for i in range(7):
            calendar_grid.columnconfigure(i, weight=1)
        
        # Configure row weights to make cells expand properly
