...
class RecycleBinModule:
    def __init__(self, parent, db, app):
        ...
        self.recycle_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.recycle_frame, text="Recycle Bin")
        
        # Create backup settings tab
        self.backup_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.backup_frame, text="Backup Settings")
        
        # Initialize recycle bin tab
        self.init_recycle_bin_tab()
        
        # Initialize backup settings tab
        self.init_backup_settings_tab()
    
    def init_recycle_bin_tab(self):
        """Initialize the recycle bin tab"""
        # Main container
        main_frame = ttk.Frame(self.recycle_frame)
        main_frame.pack(fill=tk.BOTH, expand=True)
        
        # Title
        title = ttk.Label(main_frame, text="RECYCLE BIN", font=("Arial", 16, "bold"))
        title.pack(pady=10)
    ...
    def init_backup_settings_tab(self):
        """Initialize the backup settings tab"""
        # Title
        title = ttk.Label(self.backup_frame, text="Backup Settings", 
                         font=("Arial", 16, "bold"))
        title.pack(pady=10)
        
        # Backup settings frame
        settings_frame = ttk.LabelFrame(self.backup_frame, text="Backup Configuration")
        settings_frame.pack(fill=tk.X, padx=20, pady=10)
        
        # Auto backup
        auto_backup_frame = ttk.Frame(settings_frame)
        auto_backup_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Checkbutton(auto_backup_frame, text="Enable automatic backup", 
                        variable=self.app.auto_backup_var).pack(side=tk.LEFT)
        ...
        interval_spinbox = ttk.Spinbox(interval_frame, from_=1, to=168, textvariable=self.app.backup_interval_var, 
                    width=5)
        interval_spinbox.pack(side=tk.LEFT, padx=5)
        
        # Set default value to 12 hours
        self.app.backup_interval_var.set("12")
        ...
        ttk.Entry(location_frame, textvariable=self.app.backup_location_var, width=30).pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Browse button
        browse_btn = ttk.Button(location_frame, text="Browse", command=self.app.browse_backup_location)
        browse_btn.pack(side=tk.LEFT, padx=5)
        
        # Manual backup button
        manual_backup_btn = ttk.Button(settings_frame, text="Create Backup Now", command=self.app.create_backup)
        manual_backup_btn.pack(pady=10)
        
        # Save button
        save_btn = ttk.Button(self.backup_frame, text="Save Settings", command=self.app.save_settings)
        save_btn.pack(pady=20)
        
        # Backup status
        status_frame = ttk.LabelFrame(self.backup_frame, text="Backup Status")
        status_frame.pack(fill=tk.X, padx=20, pady=10)
        
        # Last backup info
        last_backup_frame = ttk.Frame(status_frame)
        last_backup_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Label(last_backup_frame, text="Last Backup:").pack(side=tk.LEFT)
        self.last_backup_label = ttk.Label(last_backup_frame, text="Never")
        self.last_backup_label.pack(side=tk.LEFT, padx=10)
        
        # Next backup info
        next_backup_frame = ttk.Frame(status_frame)
        next_backup_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Label(next_backup_frame, text="Next Backup:").pack(side=tk.LEFT)
        self.next_backup_label = ttk.Label(next_backup_frame, text="Not scheduled")
        self.next_backup_label.pack(side=tk.LEFT, padx=10)
    ...
    def update_backup_status(self):
        ...
        if os.path.exists(config_file):
            try:
                ...
                if 'backup' in config and 'last_backup' in config['backup']:
                    last_backup_date_str = config['backup']['last_backup']
                    last_backup_date = datetime.strptime(last_backup_date_str, "%Y-%m-%d %H:%M:%S")
                    self.last_backup_label.config(text=last_backup_date.strftime("%d/%m/%Y %H:%M"))
                    
                    # Calculate next backup time
                    if self.app.auto_backup_var.get():
                        interval_hours = int(self.app.backup_interval_var.get())
                        next_backup = last_backup_date + timedelta(hours=interval_hours)
                        self.next_backup_label.config(text=next_backup.strftime("%d/%m/%Y %H:%M"))
                    else:
                        self.next_backup_label.config(text="Disabled")
                else:
                    self.last_backup_label.config(text="Never")
                    self.next_backup_label.config(text="Not scheduled")
            except Exception as e:
                print(f"Error updating backup status: {e}")
                self.last_backup_label.config(text="Error")
                self.next_backup_label.config(text="Unknown")
