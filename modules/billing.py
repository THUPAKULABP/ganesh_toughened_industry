...
class BillingModule:
    def __init__(self, parent, db, app):
        ...
        self.current_pending_invoice_id = None
    ...
    def create_ui(self):
        ...
        self.notebook.add(pending_tab, text="PI (Pending Invoices)")
        ...
        # Create pending invoices tab
        self.create_pending_invoices_tab(pending_tab)
    ...
    def create_form_content(self, parent):
        """Create the actual form content"""
        ...
        self.ppay_var = tk.StringVar(value="7013374872")
        ...
        # Bank details
        bank_name = settings.get("bank_name", "INDIAN BANK")
        bank_account = settings.get("bank_account", "7347800060")
        bank_ifsc = settings.get("bank_ifsc", "IDIB000M001")
        bank_branch = settings.get("bank_branch", "Madanapalle")
        ...
        # UPI payment button
        upi_id = settings.get("upi_id", "7013374872-3@axl")
        ...
        # Save button
        self.save_button = ttk.Button(action_frame, text="SAVE BILL", command=self.save_bill, style="Success.TButton")
        self.save_button.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Save as Pending button
        self.save_pending_btn = ttk.Button(action_frame, text="SAVE AS PENDING", command=self.save_pending_bill, style="Info.TButton")
        self.save_pending_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
    ...
    def create_bill_history(self, parent):
        ...
        print_btn.pack_forget()  # Hide the button
    ...
    def create_pending_invoices_tab(self, parent):
        """Create the pending invoices tab"""
        pending_frame = ttk.Frame(parent)
        pending_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # Title
        title = ttk.Label(pending_frame, text="PENDING INVOICES", style="Title.TLabel")
        title.pack(pady=10)
        
        # Search frame
        search_frame = ttk.Frame(pending_frame)
        search_frame.pack(fill=tk.X, padx=5, pady=5)
        
        # Customer filter
        ttk.Label(search_frame, text="Customer:").pack(side=tk.LEFT, padx=5)
        self.pending_customer_var = tk.StringVar()
        self.pending_customer_combo = ttk.Combobox(search_frame, textvariable=self.pending_customer_var)
        self.pending_customer_combo.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Date range
        ttk.Label(search_frame, text="From:").pack(side=tk.LEFT, padx=5)
        self.pending_from_var = tk.StringVar()
        ttk.Entry(search_frame, textvariable=self.pending_from_var, width=10).pack(side=tk.LEFT, padx=5)
        
        ttk.Label(search_frame, text="To:").pack(side=tk.LEFT, padx=5)
        self.pending_to_var = tk.StringVar()
        ttk.Entry(search_frame, textvariable=self.pending_to_var, width=10).pack(side=tk.LEFT, padx=5)
        
        # Search button
        search_btn = ttk.Button(search_frame, text="Search", command=self.search_pending_invoices)
        search_btn.pack(side=tk.LEFT, padx=5)
        
        # Pending invoices treeview
        self.pending_tree = ttk.Treeview(pending_frame, columns=("invoice_number", "date", "customer", "total", "status"), show="headings")
        self.pending_tree.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Define headings
        self.pending_tree.heading("invoice_number", text="Invoice #")
        self.pending_tree.heading("date", text="Date")
        self.pending_tree.heading("customer", text="Customer")
        self.pending_tree.heading("total", text="Total")
        self.pending_tree.heading("status", text="Status")
        
        # Define columns
        self.pending_tree.column("invoice_number", width=100)
        ...
        # Bind double-click to view pending invoice (removed edit functionality)
        self.pending_tree.bind("<Double-1>", self.view_pending_invoice)
        
        # Added keyboard delete support
        self.pending_tree.bind("<Delete>", lambda e: self.delete_pending_invoice())
        
        # Action buttons
        action_frame = ttk.Frame(pending_frame)
        action_frame.pack(fill=tk.X, padx=5, pady=5)
        
        # View pending invoice button
        view_btn = ttk.Button(action_frame, text="View Invoice", command=self.view_pending_invoice)
        view_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Confirm pending invoice button
        confirm_btn = ttk.Button(action_frame, text="Confirm Invoice", command=self.confirm_pending_invoice)
        confirm_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        # Delete pending invoice button
        delete_btn = ttk.Button(action_frame, text="Delete Invoice", command=self.delete_pending_invoice)
        delete_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
    ...
    def load_settings(self):
        ...
        try:
            ...
            if not os.path.exists(settings_path):
                settings_path = "ganesh_toughened_industry/config/settings.json"
            ...
            if os.path.exists(settings_path):
                ...
                return {
                    "company_name": "GANESH TOUGHEN INDUSTRY",
                    "company_address": "Plot No B13, Madanapalle Industrial Estate, Kammapalle, Madanapalle, Annamayya district, Andhra Pradesh state - 517325",
                    "company_phone": "9398530499, 7013374872",
                    "company_gst": "37EXFPK2395C1ZE",
                    "upi_id": "7013374872-3@axl",
                    "upi_name": "GANESH TOUGHENED INDUSTRY",
                    "bank_name": "INDIAN BANK",
                    "bank_account": "7347800060",
                    "bank_ifsc": "IDIB000M001",
                    "bank_branch": "Madanapalle",
                    "invoice_prefix": "GTI",
                    "backup_enabled": True,
    ...
    def load_customers(self):
        ...
        # Also load for pending tab if it exists
        if hasattr(self, 'pending_customer_combo'):
            self.pending_customer_combo["values"] = customer_names
    ...
    def get_extra_charges_breakdown(self):
        """Get extra charges breakdown as JSON string"""
        try:
            extra_charges = {
                "cutout": float(self.cutout_var.get()),
                "hole": float(self.hole_var.get()),
            ...
            return json.dumps(extra_charges)
        except ValueError:
    ...
    def save_bill(self):
        """Save bill to database or update existing bill"""
        ...
        try:
            ...
            extra_charges_breakdown = self.get_extra_charges_breakdown()
            
            # Initialize invoice_number
            invoice_number = self.invoice_number_var.get()
            
            # Check if we're updating an existing invoice
            if hasattr(self, 'edit_mode') and self.edit_mode and hasattr(self, 'current_invoice_id'):
                ...
                try:
                    cursor.execute("""
                    UPDATE invoices SET 
                        customer_id = ?, date = ?, subtotal = ?, extra_charges = ?, 
                        round_off = ?, total = ?, payment_mode = ?, p_pay_no = ?,
                    ...
                    for item in self.invoice_items:
                        ...
                        """, (
                            ...
                            item["actual_height"],
                            item["actual_width"],
                            item["chargeable_height"],
                            item["chargeable_width"],
                            item["sqft"],
                            item["rounded_sqft"],
                            item["rate"],
                            item["amount"],
                    ...
                    if payment_row:
                        ...
                        cursor.execute("""
                        UPDATE payments SET 
                            customer_id = ?, date = ?, amount = ?, mode = ?, reference = ?
                        WHERE payment_id = ?
                        """, (
                            self.selected_customer["customer_id"],
                            invoice_date.strftime("%Y-%m-%d"),
                    ...
                    # Update works
                    cursor.execute("DELETE FROM works WHERE invoice_id = ?", (invoice_id,))
                    ...
                    for item in self.invoice_items:
                        cursor.execute("""
                        INSERT INTO inventory (product_id, date, type, quantity)
                        VALUES (?, ?, ?, ?)
                        """, (
                            item["product_id"],
                            invoice_date.strftime("%Y-%m-%d"),
                    ...
                    # Change button text back to "SAVE BILL"
                    if hasattr(self, 'save_button'):
                        self.save_button.config(text="SAVE BILL")
                    
                    # Show SAVE AS PENDING button again
                    if hasattr(self, 'save_pending_btn'):
                        self.save_pending_btn.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
                    ...
                    conn.rollback()
                    conn.close()
                    messagebox.showerror("Error", f"Could not update bill: {e}")
                    return
                ...
                # Extract invoice_id and invoice_number from the result
                invoice_id, invoice_number = result
            ...
            self.last_saved_invoice = {
                "invoice_id": invoice_id if not hasattr(self, 'edit_mode') or not self.edit_mode else self.current_invoice_id,
                "invoice_number": invoice_number,
    ...
    def save_pending_bill(self):
        """Save bill as pending to database or update existing pending invoice"""
        if not self.selected_customer:
            messagebox.showerror("Error", "Please select a customer")
            return
        ...
        try:
            ...
            if hasattr(self, 'edit_mode') and self.edit_mode and hasattr(self, 'current_pending_invoice_id'):
                # Update existing pending invoice
                pending_invoice_id = self.current_pending_invoice_id
                
                # Get current invoice number to maintain consistency
                invoice_number = self.invoice_number_var.get()
                
                # Update pending invoice
                result = self.db.update_pending_invoice(
                ...
                if not result:
                    messagebox.showerror("Error", "Could not update pending invoice")
                    return
                
                # Extract the invoice number from the result (in case it was changed)
                success, invoice_number = result
                
                # Update pending invoice items
                success = self.db.update_pending_invoice_items(
                    pending_invoice_id=pending_invoice_id,
                    items=self.invoice_items
                )
                
                if not success:
                    messagebox.showerror("Error", "Could not update pending invoice items")
                    return
                
                messagebox.showinfo("Success", "Pending invoice updated successfully")
                ...
                # Add pending invoice to database
                result = self.db.add_pending_invoice(
                    customer_id=self.selected_customer["customer_id"],
                    date=invoice_date,
                    invoice_number=invoice_number,
                    subtotal=subtotal,
                ...
                if not result:
                    messagebox.showerror("Error", "Could not save pending invoice")
                    return
                
                # Extract pending_invoice_id and invoice_number from the result
                pending_invoice_id, invoice_number = result
                
                # Add pending invoice items
                for item in self.invoice_items:
                    self.db.add_pending_invoice_item(
                        pending_invoice_id=pending_invoice_id,
                        product_id=item["product_id"],
                        actual_height=item["actual_height"],
                        actual_width=item["actual_width"],
                ...
                messagebox.showinfo("Success", "Pending bill saved successfully")
            
            # Store the last saved invoice for PDF generation
            self.last_saved_invoice = {
                "pending_invoice_id": pending_invoice_id if not hasattr(self, 'edit_mode') or not self.edit_mode else self.current_pending_invoice_id,
                "invoice_number": invoice_number,
                "customer": self.selected_customer,
                "items": self.invoice_items,
                "date": invoice_date,
            ...
            print(f"Error in save_pending_bill: {traceback.format_exc()}")
            messagebox.showerror("Error", f"Could not save pending bill: {e}")
    ...
    def _generate_pdf_common(self, invoice_number, customer, items, invoice_date, subtotal, total_extra, total, payment_mode, ppay_no, extra_charges_breakdown=None, is_pending=False):
        ...
        try:
            ...
            prefix = "Pending_Invoice_" if is_pending else "Invoice_"
            safe_invoice_number = invoice_number.replace('/', '_')
            file_path = os.path.join("invoices", f"{prefix}{safe_invoice_number}.pdf")
            
            # Generate the PDF
            success = self._generate_invoice_pdf_to_path(
                invoice_number, customer, items, invoice_date, 
                subtotal, total_extra, total, payment_mode, ppay_no, file_path, extra_charges_breakdown, is_pending
            )
    ...
    def _generate_pdf_from_saved_invoice(self):
        ...
        is_pending = 'pending_invoice_id' in self.last_saved_invoice
        extra_charges_breakdown = self.last_saved_invoice.get('extra_charges_breakdown')
    ...
    def print_bill(self):
        ...
        success = self._generate_invoice_pdf_to_path(
            ...
            invoice['p_pay_no'],
            file_path,
            invoice.get('extra_charges_breakdown')
        )
    ...
    def edit_bill(self):
        ...
        # Set edit mode
        self.edit_mode = True
        self.current_invoice_id = invoice['invoice_id']
        
        # Change save button text
        if hasattr(self, 'save_button'):
            self.save_button.config(text="UPDATE BILL")
        
        # Hide the SAVE AS PENDING button when editing a saved bill
        if hasattr(self, 'save_pending_btn'):
            self.save_pending_btn.pack_forget()
    ...
    def new_bill(self):
        ...
        # Change save button text back to "SAVE BILL"
        if hasattr(self, 'save_button'):
            self.save_button.config(text="SAVE BILL")
    ...
    def _generate_invoice_pdf_to_path(self, invoice_number, customer, items, invoice_date, subtotal, total_extra, total, payment_mode, ppay_no, file_path, extra_charges_breakdown=None, is_pending=False):
        ...
        try:
            ...
            # Company details
            company_name = settings.get("company_name", "GANESH TOUGHEN INDUSTRY")
            company_address = settings.get("company_address", "Plot no:B13, Industrial Estate, Madanapalli")
            company_phone = settings.get("company_phone", "9398530499, 7013374872")
            company_gst = settings.get("company_gst", "37EXFPK2395C1ZE")
            ...
            if is_pending:
                invoice_title = Paragraph("<b>PENDING INVOICE</b>", styles["CustomHeading"])
            else:
            ...
            if is_pending:
                invoice_data.append(["Status:", "PENDING"])
            ...
            # Parse extra charges breakdown if provided
            extra_charges = {}
            ...
            payment_layout = Table([
                [
                    ...
                    [
                        ...
                        Spacer(1, 0.1*inch),
                        Paragraph(f"Bank: {settings.get('bank_name', '')}", styles["CustomNormal"]),
                        Paragraph(f"Account: {settings.get('bank_account', '')}", styles["CustomNormal"]),
                        Paragraph(f"IFSC: {settings.get('bank_ifsc', '')}", styles["CustomNormal"]),
                        Paragraph(f"Branch: {settings.get('bank_branch', '')}", styles["CustomNormal"])
                    ],
    ...
    def open_upi_app(self):
        ...
        try:
            ...
            # Get UPI name
            upi_name = settings.get("upi_name", "GANESH TOUGHENED INDUSTRY")
    ...
    # Pending Invoices Methods
    def load_pending_invoices(self):
        """Load pending invoices into the treeview"""
        # Clear the treeview
    ...
    def search_pending_invoices(self):
        """Search pending invoices based on criteria"""
        customer = self.pending_customer_var.get()
        from_date_str = self.pending_from_var.get()
        to_date_str = self.pending_to_var.get()
        ...
        pending_invoices = self.db.search_pending_invoices(customer, from_date, to_date)
    ...
    def view_pending_invoice(self, event=None):
        """View the selected pending invoice"""
        ...
        # Get the invoice details
        invoice = self.db.get_pending_invoice_by_number(invoice_number)
        if not invoice:
            messagebox.showerror("Error", "Invoice not found")
            return
        
        # Get the invoice items
        items = self.db.get_pending_invoice_items(invoice['pending_invoice_id'])
        
        # Generate the PDF
        safe_invoice_number = invoice_number.replace('/', '_')
        file_path = os.path.join("invoices", f"Pending_Invoice_{safe_invoice_number}.pdf")
        ...
        if not os.path.exists(file_path):
            ...
            success = self._generate_invoice_pdf_to_path(
                ...
                file_path,
                invoice.get('extra_charges_breakdown'),
                is_pending=True  # Mark as pending invoice
            )
    ...
    def confirm_pending_invoice(self):
        """Confirm the selected pending invoice"""
        selected = self.pending_tree.selection()
        if not selected:
            messagebox.showerror("Error", "Please select an invoice to confirm")
            return
        ...
        # Confirm with user
        if not messagebox.askyesno("Confirm Invoice", f"Are you sure you want to confirm invoice {invoice_number}?"):
            return
        ...
        # Convert pending invoice to regular invoice
        try:
            ...
            if not result:
                messagebox.showerror("Error", "Could not confirm invoice")
                return
            ...
            # Add payment if payment mode is specified
            ...
            # Update pending invoice status to confirmed
            self.db.update_pending_invoice_status(invoice['pending_invoice_id'], 'confirmed')
            
            messagebox.showinfo("Success", "Invoice confirmed successfully")
            
            # Refresh both pending and regular invoice lists
            self.load_pending_invoices()
            self.load_bill_history()
            
        except Exception as e:
            print(f"Error confirming pending invoice: {e}")
            messagebox.showerror("Error", f"Could not confirm invoice: {e}")
    
    def delete_pending_invoice(self):
        """Delete the selected pending invoice"""
        selected = self.pending_tree.selection()
        if not selected:
        ...
        # Add to recycle bin before deletion
        self.db.add_to_recycle_bin("pending_invoices", invoice["pending_invoice_id"], invoice_data)
        
        # Delete the pending invoice from the database
        conn = self.db.get_connection()
        cursor = conn.cursor()
        
        try:
            # Delete pending invoice items
            cursor.execute("DELETE FROM pending_invoice_items WHERE pending_invoice_id = ?", (invoice["pending_invoice_id"],))
            
            # Delete the pending invoice
            cursor.execute("DELETE FROM pending_invoices WHERE pending_invoice_id = ?", (invoice["pending_invoice_id"],))
            
            conn.commit()
            
            conn.close()
            
            # Refresh the pending invoice list
            self.load_pending_invoices()
            
            messagebox.showinfo("Success", "Pending invoice moved to recycle bin")
        except Exception as e:
            conn.rollback()
            conn.close()
            messagebox.showerror("Error", f"Could not delete pending invoice: {e}")